window.onload = function () {
	...
	var maxPokeNumber =
		localStorage.getItem('maxPokeNr') != null && localStorage.getItem('maxPokeNr') > 0
			? localStorage.getItem('maxPokeNr')
			: 0
	...
	function fetchPokemon(fetchurl, createCard) {
		//Esta función deberia solo traer los datos y entregarlos
		// Las operaciones/validaciones que haces aqui se deberían hacer en la funcion que pinta los pokemon.
	...
	function filterCards(e) {
		...
		// No se está controlando la función por defecto de los formularios
		// cuando le das buscar(enter) se recarga la página.
	...
	function getAvailablePokemons(fetchurl, loadCards) {
		// Aqui igual, cada función deberia hacer solo una cosa
		// si la función devuelve los pokemon disponibles solo debería hacer esa tarea.
	...
	function switchTheme(newTheme) {
		switch (newTheme) {
			...
			//Para no repetir código puede poner dos casos juntos en el switch asi:
			case 'light':
			default:
				body.setAttribute('data-theme', 'light')
				nodes.forEach((node) => {
